...
export const GET = async (req: Request) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`SELECT TOP 1 proofAddress, proof_filename FROM CompaniesAddress WHERE id = @id AND holderEmail = @holderEmail`);

        if (result.recordset.length > 0) {
            
            const fileData = result.recordset[0].proofAddress;
            const fileName = result.recordset[0].proof_filename || "downloaded_file"; // Use the provided filename or a default one
