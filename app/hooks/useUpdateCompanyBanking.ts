...
const useUpdateCompanyBanking = () => {
    ...
    const submitForm = async (e: FormEvent<HTMLFormElement>,bank:string,branch:string,branchcode:string,
        ...
        try {
            ...
            const response = await axios.patch('/api/upload/company/banking', formData, {
                headers: {
            ...
            if (response.status === 200) {
                ...
                queryClient.invalidateQueries({ queryKey: ["companyBanking"] });
                //handleSigOut();
            } else {
...
export default useUpdateCompanyBanking;